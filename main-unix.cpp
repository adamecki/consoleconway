...
int main() {
    ...
    while(true) {
        drawBoard();
        cout << "q - rozpocznij; c - wyczysc; r - rozmiesc losowo" << endl;
        cout << "Aktywuj lub deaktywuj pole (x,y): ";
        int x = -1; int y = -1;

        getline(cin, input);

        if(input[0] == 'c') {clear();}
        if(input[0] == 'r') {randomize();}
        if(input[0] == 'q') {break;}
    ...
    while(true) {
        drawBoard();
        cout << "Zyje " << alive << " komorek." << endl;

        if(alive == 0) {
            cout << "Koniec symulacji! Wszystkie komorki obumarly" << endl;
            break;
        }
        ...
        for(int i = 0; i < board_y; i++) {
            for(int j = 0; j < board_x; j++) {
                if(!(board[i][j]) && (countClosest(j, i) == 3)) {
                    to_activate.push_back({j, i});
                    alive++;
                } else if (board[i][j] && (countClosest(j, i) < 2 || countClosest(j, i) > 3)) {
